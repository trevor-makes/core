...
// TODO move to util or data structs header?
...
// TODO move to util or interrupts header?
class ISRGuard {
#ifdef __AVR_ARCH__
  uint8_t sreg;
public:
...
public:
  ...
  bool get(uint8_t index, uint16_t& time, Args&... args) {
    if (index < count_) {
      ...
      util::copy_into_args(frames_[index].values, args...);
      return true;
    } else {
      return false;
    }
  ...
  bool insert(uint16_t time, const Args... args) {
    ...
    for (; index < count_; ++index) {
      if (time == frames_[index].time) {
        ...
        util::copy_from_args(frames_[index].values, args...);
        return true;
