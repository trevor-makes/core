...
uint8_t write_pfx_code(uint16_t addr, uint8_t prefix, uint8_t code) {
  ...
  if (has_prefix) API::BUS::write_bus(addr++, prefix);
...
uint8_t write_pfx_code_idx(uint16_t addr, uint8_t prefix, uint8_t code, Operand& index) {
  ...
  if (has_index) API::BUS::write_bus(addr + size, index.value);
...
uint8_t write_code_word(uint16_t addr, uint8_t code, uint16_t data) {
  API::BUS::write_bus(addr, code);
  API::BUS::write_bus(addr + 1, data & 0xFF);
  API::BUS::write_bus(addr + 2, data >> 8);
  return 3;
}
...
uint8_t write_cb_code(uint16_t addr, uint8_t code, Operand& op) {
  ...
  if (prefix != 0) {
    ...
    API::BUS::write_bus(addr, prefix);
    API::BUS::write_bus(addr + 1, PREFIX_CB);
    API::BUS::write_bus(addr + 2, op.value);
    API::BUS::write_bus(addr + 3, code | reg);
    return 4;
  } else {
...
uint8_t write_ld(uint16_t addr, Operand& dst, Operand& src) {
  ...
  if (dst_reg != REG_INVALID) {
    ...
    // LD r,n
      ...
      API::BUS::write_bus(addr + size, src.value);
      return size + 1;
    }
